      ...
      <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:p-7">
        <ConsoleCard title="Detail Paket">
          <form className="space-y-5">
            <div><label className={labelCls}>Judul Try Out</label><input className={inputCls} placeholder="mis. Try Out Nasional #5" /></div>
            <div className="grid gap-5 sm:grid-cols-2">
              <div><label className={labelCls}>Tipe</label><select className={inputCls}><option>SNBT</option><option>SKD CPNS</option><option>Mandiri</option></select></div>
              <div><label className={labelCls}>Durasi (menit)</label><input type="number" className={inputCls} defaultValue={100} /></div>
            </div>
            <div className="grid gap-5 sm:grid-cols-2">
              <div><label className={labelCls}>Jumlah Soal</label><input type="number" className={inputCls} defaultValue={20} /></div>
              <div><label className={labelCls}>Harga (Rp)</label><input type="number" className={inputCls} defaultValue={0} /></div>
            </div>
            <div><label className={labelCls}>Pemilihan Soal</label>
              <div className="rounded-lg border border-dashed bg-muted/40 p-4 text-body-sm text-ink-muted">Pilih soal dari bank ({totalSoal} tersedia) atau gunakan Blueprint untuk auto-generate. <span className="text-ink-body">Picker soal terhubung saat bank soal terisi.</span></div>
            </div>
